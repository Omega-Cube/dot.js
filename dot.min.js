...
if(typeof G_vmlCanvasManager!=="undefined")G_vmlCanvasManager.initElement(this.canvas),this.canvas=document.getElementById(this.canvas.id);this.container.appendChild(this.elements);this.ctx=this.canvas.getContext("2d");this.scale=1;this.minimumScale=0.5;this.maximumScale=2;this.padding=8;this.dashLength=6;this.dotSpacing=4;this.graphs=[];this.images={};this.numImagesFinished=this.numImages=0;this.graphLoaded=false;this.allowMouseScrolling=true;b&&this.load(b,c);DotUtils.addEventHandler(this.container,
"mousewheel",DotUtils.bind(this,this._handleMouseWheel))},setScale:function(a){this.scale=+a;if(this.scale<this.minimumScale)this.scale=this.minimumScale;if(this.scale>this.maximumScale)this.scale=this.maximumScale;this.graphLoaded&&this.draw()},getScale:function(){return this.scale},setMinimumScale:function(a){a=+a;if(a>this.maximumScale)a=this.maximumScale;a<=0&&(a=0.001);this.minimumScale=a;this.scale<a&&this.setScale(a)},getMinimumScale:function(){return this.minimumScale},setMaximumScale:function(a){a=
+a;if(a<this.minimumScale)a=this.minimumScale;this.maximumScale=a;this.scale>this.maximumScale&&this.setScale(this.maximumScale)},getMaximumScale:function(){return this.maximumScale},setAllowMouseScrolling:function(a){this.allowMouseScrolling=!!a},getAllowMouseScrolling:function(){return this.allowMouseScrolling},setImagePath:function(a){this.imagePath=a},load:function(a,b){document.getElementById("debug_output").innerHTML="";var c=DotUtils.bind(this,function(a){this.parse(a)});DotUtils.ajaxGet(a,
b,c)},parse:function(a){this.graphs=[];this.height=this.width=0;this.bbEnlarge=this.maxHeight=this.maxWidth=false;this.bbScale=1;this.dpi=96;this.bgcolor={opacity:1};this.bgcolor.canvasColor=this.bgcolor.textColor="#ffffff";for(var a=a.split(/\r?\n/),b=0,c,d,e,g,h,f,i,l,k,m,j=[];b<a.length;)if(c=a[b++].replace(/^\s+/,""),""!=c&&"#"!=c.substr(0,1)){for(;b<a.length&&";"!=(d=c.substr(c.length-1,c.length))&&"{"!=d&&"}"!=d;)"\\"==d&&(c=c.substr(0,c.length-1)),c+=a[b++];if(0==j.length){if(e=c.match(this.graphMatchRe))g=
new DotGraph(e[3],this),j.unshift(g),j[0].strict=typeof e[1]!=="undefined",j[0].type="graph"==e[2]?"undirected":"directed",j[0].attrs.xdotversion="1.0",this.graphs.push(j[0])}else if(e=c.match(this.subgraphMatchRe))j.unshift(new DotGraph(e[1],this,g,j[0])),j[1].subgraphs.push(j[0]);if(!e)if("}"==c){if(j.shift(),0==j.length)break}else{if(e=c.match(this.nodeMatchRe))switch(f=e[2],i=e[5],m=j[0].drawAttrs,h=false,f){case "graph":k=j[0].attrs;h=true;break;case "node":k=j[0].nodeAttrs;break;case "edge":k=
j[0].edgeAttrs;break;default:c=new DotNode(f,this,g,j[0]),k=c.attrs,m=c.drawAttrs,j[0].nodes.push(c)}else if(e=c.match(this.edgeMatchRe))f=e[1],i=e[8],c=new DotEdge(f,this,g,j[0],e[2],e[5]),k=c.attrs,m=c.drawAttrs,j[0].edges.push(c);if(e){do{if(0==i.length)break;if(e=i.match(this.attrMatchRe)){if(i=i.substr(e[0].length),c=e[1],l=this.unescape(e[2]),/^_.*draw_$/.test(c)?m[c]=l:k[c]=l,h&&1==j.length)switch(c){case "bb":c=l.split(/,/);this.width=Number(c[2]);this.height=Number(c[3]);break;case "bgcolor":this.bgcolor=
g.parseColor(l);break;case "dpi":this.dpi=l;break;case "size":(c=l.match(/^(\d+|\d*(?:\.\d+)),\s*(\d+|\d*(?:\.\d+))(!?)$/))?(this.maxWidth=72*Number(c[1]),this.maxHeight=72*Number(c[2]),this.bbEnlarge="!"==c[3]):debug("can't parse size");break;case "xdotversion":0>this.versionCompare(this.maxXdotVersion,k.xdotversion)&&debug("unsupported xdotversion "+k.xdotversion+"; this script currently supports up to xdotversion "+this.maxXdotVersion)}}else debug("can't read attributes for entity "+f+" from "+
i)}while(e)}}}this.draw();this.graphLoaded=true},draw:function(a){typeof a==="undefined"&&(a=false);var b=this.scale*this.dpi/72,c=Math.round(b*this.width+2*this.padding),d=Math.round(b*this.height+2*this.padding);if(!a){this.canvas.width=c;this.canvas.height=d;this.canvas.style.width=c+"px";this.canvas.style.height=d+"px";for(this.container.style.width=c+"px";this.elements.firstChild;)this.elements.removeChild(this.elements.firstChild)}this.ctx.save();this.ctx.lineCap="round";this.ctx.fillStyle=
this.bgcolor.canvasColor;this.ctx.fillRect(0,0,c,d);this.ctx.translate(this.padding,this.padding);this.ctx.scale(b,b);this.graphs[0].draw(this.ctx,b,a);this.ctx.restore()},drawPath:function(a,b,c,d){c&&(a.beginPath(),b.makePath(a),a.fill());if(a.fillStyle!=a.strokeStyle||!c){switch(d){case "dashed":a.beginPath();b.makeDashedPath(a,this.dashLength);break;case "dotted":var e=a.lineWidth;a.lineWidth*=2;a.beginPath();b.makeDottedPath(a,this.dotSpacing);break;default:c||(a.beginPath(),b.makePath(a))}a.stroke();
if(e)a.lineWidth=e}},unescape:function(a){var b=a.match(/^"(.*)"$/);return b?b[1].replace(/\\"/g,'"'):a},parseHexColor:function(a){if(a=a.match(/^#([0-9a-f]{2})\s*([0-9a-f]{2})\s*([0-9a-f]{2})\s*([0-9a-f]{2})?$/i)){var b,c="#"+a[1]+a[2]+a[3],d=1;a[4]?(d=parseInt(a[4],16)/255,b="rgba("+parseInt(a[1],16)+","+parseInt(a[2],16)+","+parseInt(a[3],16)+","+d+")"):b=c}return{canvasColor:b,textColor:c,opacity:d}},hsvToRgbColor:function(a,b,c){var d,e,g,h,f,i;a*=360;d=Math.floor(a/60)%6;e=a/60-d;a=c*(1-b);
g=c*(1-e*b);b=c*(1-(1-e)*b);switch(d){case 0:h=c;f=b;i=a;break;case 1:h=g;f=c;i=a;break;case 2:h=a;f=c;i=b;break;case 3:h=a;f=g;i=c;break;case 4:h=b;f=a;i=c;break;case 5:h=c,f=a,i=g}return"rgb("+Math.round(255*h)+","+Math.round(255*f)+","+Math.round(255*i)+")"},versionCompare:function(a,b){for(var a=a.split("."),b=b.split("."),c,d;a.length||b.length;){c=a.length?a.shift():0;d=b.length?b.shift():0;if(c<d)return-1;if(c>d)return 1}return 0},_handleMouseWheel:function(a){if(this.allowMouseScrolling){if(!a)a=
window.event;var b=0;a.wheelDelta?(b=a.wheelDelta/120,window.opera&&(b=-b)):a.detail&&(b=-a.detail/3);b&&this.setScale(this.getScale()+b/10);return DotUtils.preventDefault(a)}},idMatch:'([a-zA-Z\u0080-\uffff_][0-9a-zA-Z\u0080-\uffff_]*|-?(?:\\.\\d+|\\d+(?:\\.\\d*)?)|"(?:\\\\"|[^"])*"|<(?:<[^>]*>|[^<>]+?)+>)'});Dot.prototype.nodeIdMatch=Dot.prototype.idMatch+"(?::"+Dot.prototype.idMatch+")?(?::"+Dot.prototype.idMatch+")?";
Dot.prototype.graphMatchRe=RegExp("^(strict\\s+)?(graph|digraph)(?:\\s+"+Dot.prototype.idMatch+")?\\s*{$","i");Dot.prototype.subgraphMatchRe=RegExp("^(?:subgraph\\s+)?"+Dot.prototype.idMatch+"?\\s*{$","i");Dot.prototype.nodeMatchRe=RegExp("^("+Dot.prototype.nodeIdMatch+")\\s+\\[(.+)\\];$");Dot.prototype.edgeMatchRe=RegExp("^("+Dot.prototype.nodeIdMatch+"\\s*-[->]\\s*"+Dot.prototype.nodeIdMatch+")\\s+\\[(.+)\\];$");
Dot.prototype.attrMatchRe=RegExp("^"+Dot.prototype.idMatch+"="+Dot.prototype.idMatch+"(?:[,\\s]+|$)");var DotTokenizer=DotUtils.createClass({initialize:function(a){this.str=a},takeChars:function(a){a||(a=1);for(var b=[];a--;){var c=this.str.match(/^(\S+)\s*/);c?(this.str=this.str.substr(c[0].length),b.push(c[1])):b.push(false)}return 1==b.length?b[0]:b},takeNumber:function(a){a||(a=1);if(1==a)return Number(this.takeChars());else{for(var b=this.takeChars(a);a--;)b[a]=Number(b[a]);return b}},takeString:function(){var a=Number(this.takeChars()),b=0,c;if("-"!=this.str.charAt(0))return false;for(;0<a;)++b,
...
new DotTokenizer(this.drawAttrs[daKey]);if(d=f.takeChars()){var i="solid";for(a.save();d;){switch(d){case "E":case "e":var l="E"==d,k=f.takeNumber();d=this.dot.height-f.takeNumber();e=f.takeNumber();var m=f.takeNumber(),k=new Ellipse(k,d,e,m);break;case "P":case "p":case "L":var l="P"==d,m="L"!=d,j=f.takeNumber();e=f.takeNumber(2*j);k=new Path;for(d=2;d<2*j;d+=2)k.addBezier([new Point(e[d-2],this.dot.height-e[d-1]),new Point(e[d],this.dot.height-e[d+1])]);m&&k.addBezier([new Point(e[2*j-2],this.dot.height-
e[2*j-1]),new Point(e[0],this.dot.height-e[1])]);break;case "B":case "b":l="b"==d;j=f.takeNumber();e=f.takeNumber(2*j);k=new Path;for(d=2;d<2*j;d+=6)k.addBezier([new Point(e[d-2],this.dot.height-e[d-1]),new Point(e[d],this.dot.height-e[d+1]),new Point(e[d+2],this.dot.height-e[d+3]),new Point(e[d+4],this.dot.height-e[d+5])]);break;case "I":d=f.takeNumber();e=this.dot.height-f.takeNumber();var m=f.takeNumber(),j=f.takeNumber(),o=f.takeString();this.dot.images[o]||(this.dot.images[o]=new DotImage(this.dot,
o));this.dot.images[o].draw(a,d,e-j,m,j);break;case "T":d=Math.round(b*f.takeNumber()+this.dot.padding);e=Math.round(b*this.dot.height+2*this.dot.padding-(b*(f.takeNumber()+this.dot.bbScale*fontSize)+this.dot.padding));m=f.takeNumber();j=Math.round(b*f.takeNumber());o=f.takeString();if(!c&&!/^\s*$/.test(o)){o=DotUtils.escapeHtml(o);do if(matches=o.match(/ ( +)/)){var q=" ";matches[1].length.times(function(){q+="&nbsp;"});o=o.replace(/  +/,q)}while(matches);var n,r=this.getAttr("URL",true)||this.getAttr("href",
true);if(r){var s=this.getAttr("target",true)||"_self",u=this.getAttr("tooltip",true)||this.getAttr("label",true);n=document.createElement("a");n.href=r;n.target=s;n.title=u;var r="onclick,onmousedown,onmouseup,onmouseover,onmousemove,onmouseout".split(","),t;for(t in r)(s=this.getAttr(t,true))&&n.setAttribute(t,s);n.style.textDecoration="none"}else n=document.createElement("span");DotUtils.setHtml(n,o);n.style.fontSize=Math.round(fontSize*b*this.dot.bbScale)+"px";n.style.fontFamily=fontFamily;n.style.color=
...
d.l||a>d.r||b<d.t||b>d.b)},isPointOnBezier:function(a,b,c){typeof c==="undefined"&&(c=0);if(!this.isPointInBB(a,b,c))return false;for(var d=this.chordPoints(),e=d[0].p,g,h,f,i,l,k,m=1;m<d.length;++m){g=d[m].p;h=e.x;f=e.y;i=g.x;l=g.y;k=new Rect(h,f,i,l);if(k.isPointInBB(a,b,c)&&(h=Math.abs(h*l+i*b+a*f-i*f-a*l-h*b),e=e.distanceFrom(g),e=h/e,e<=c))return true;e=g}return false},controlPolygonLength:function(){for(var a=0,b=1;b<this.order;++b)a+=this.points[b-1].distanceFrom(this.points[b]);return(this.controlPolygonLength=
...
c;)f.remainingLength-=c,g+=h,e.push(g),a!=c&&(c=a,h=c/f.length*f.dt);c-=f.remainingLength;g=f.tEnd}return{times:e,nextDistance:c}},coefficients:function(){function a(a){for(;a.length>1;){for(var b=Array(a.length-1),c=0;c<a.length-1;++c){var d=b,e=c,g=a[c],j=a[c+1];g.push(0);var o=Array(g.length);o[0]=g[0];for(var q=0;q<j.length;++q)o[q+1]=g[q+1]+j[q]-g[q];d[e]=o}a=b}return a[0]}for(var b=[],c=[],d=0,e;e=this.points[d++];)b.push([e.x]),c.push([e.y]);var g={xs:a(b),ys:a(c)};return(this.coefficients=

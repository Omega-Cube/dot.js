...
DotUtils.ajaxGet=function(a,b,c){var d=null;if(window.XMLHttpRequest)d=new window.XMLHttpRequest;else try{d=new ActiveXObject("MSXML2.XMLHTTP.3.0")}catch(e){d=null}d===null&&alert("Your browser does not support basic AJAX requests");if(typeof b!=="undefined"&&b!==null){var g=false;a.indexOf("?")>-1&&(idFirst=false);for(p in b)g?(a+="?",g=false):a+="&",a+=escape(p)+"="+escape(b[p])}try{d.open("GET",a,true)}catch(h){d=new ActiveXObject("Microsoft.XMLHTTP"),d.open("GET",a,true)}d.onreadystatechange=
function(){d.readyState==4&&(d.status==200?c(d.responseText):d.status==0&&(alert("It seems you have some cross-domain access issues. Dot.js does not support cross-domain loading (yet ?). I'll still try to load your data but the app might get unstable."),c(d.responseText)))};d.send()};DotUtils.extend=function(a,b){for(var c in b)a[c]=b[c];return a};
DotUtils.addEventHandler=function(a,b,c){a.addEventListener?(b=="mousewheel"&&a.addEventListener("DOMMouseScroll",c,false),a.addEventListener(b,c,false)):a.attachEvent&&a.attachEvent("on"+b,c)};DotUtils.removeEventHandler=function(a,b,c){a.removeEventHandler?(b=="mousewheel"&&a.removeEventHandler("DOMMouseScroll",c,false),a.removeEventHandler(b,c,false)):a.detachEvent&&a.detachEvent("on"+b,c)};
...
DotUtils.addMethods=function(a,b){var c=a.superclass&&a.superclass.prototype,d=[],e;for(e in b)d.push(e);for(var g=0,h=d.length;g<h;g++){e=d[g];var f=b[e];if(c&&typeof f==="function"&&DotUtils.argumentNames(f)[0]=="$super")var j=f,f=function(a){return function(){return c[a].apply(this,arguments)}}(e),f=DotUtils.wrap(f,j);a.prototype[e]=f}return a};
DotUtils.argumentNames=function(a){a=a.toString().match(/^[\s\(]*function[^(]*\(([^\)]*)\)/)[1].replace(/\s+/g,"").split(",");return a.length==1&&!a[0]?[]:a};DotUtils.wrap=function(a,b){return function(){return b.apply(this,[DotUtils.bind(this,a)].concat(Array.prototype.slice.call(arguments)))}};var Dot=DotUtils.createClass({maxXdotVersion:"1.2",colors:{fallback:{black:"000000",lightgrey:"d3d3d3",white:"ffffff"}},initialize:function(a,b,c){this._canvas=document.createElement("canvas");this._canvas.style.position="absolute";if(!Dot._canvasCounter)Dot._canvasCounter=0;this._canvas.id="dot_canvas_"+ ++Dot._canvasCounter;this._container=document.getElementById(a);this._container.style.position="relative";this._container.style.overflow="hidden";this._container.appendChild(this._canvas);this._cacheCanvas=
...
this._numImages=0;this._imagePath="";this._graphLoaded=false;this._allowMouseScrolling=true;b&&this.load(b,c);DotUtils.addEventHandler(this._container,"mousewheel",DotUtils.bind(this,this._handleMouseWheel))},setScale:function(a){this._scale=+a;if(this._scale<this._minimumScale)this._scale=this._minimumScale;if(this._scale>this._maximumScale)this._scale=this._maximumScale;this._graphLoaded&&this._draw()},getScale:function(){return this._scale},setMinimumScale:function(a){a=+a;if(a>this._maximumScale)a=
...
getHeight:function(){return this._screenHeight},load:function(a,b){document.getElementById("debug_output").innerHTML="";var c=DotUtils.bind(this,function(a){this.parse(a)});DotUtils.ajaxGet(a,b,c)},parse:function(a){this._graphs=[];this._height=this._width=0;this._bbEnlarge=this._maxHeight=this._maxWidth=false;this._dpi=96;this._bgcolor={opacity:1};this._bgcolor.canvasColor=this._bgcolor.textColor="#ffffff";for(var a=a.split(/\r?\n/),b=0,c,d,e,g,h,f,j,l,k,m,i=[];b<a.length;)if(c=a[b++].replace(/^\s+/,
""),""!=c&&"#"!=c.substr(0,1)){for(;b<a.length&&";"!=(d=c.substr(c.length-1,c.length))&&"{"!=d&&"}"!=d;)"\\"==d&&(c=c.substr(0,c.length-1)),c+=a[b++];if(0==i.length){if(e=c.match(this._graphMatchRe))g=new DotGraph(e[3],this),i.unshift(g),i[0]._strict=typeof e[1]!=="undefined",i[0]._type="graph"==e[2]?"undirected":"directed",i[0]._attrs.xdotversion="1.0",this._graphs.push(i[0])}else if(e=c.match(this._subgraphMatchRe))i.unshift(new DotGraph(e[1],this,g,i[0])),i[1]._subgraphs.push(i[0]);if(!e)if("}"==
c){if(i.shift(),0==i.length)break}else{if(e=c.match(this._nodeMatchRe))switch(f=e[2],j=e[5],m=i[0]._drawAttrs,h=false,f){case "graph":k=i[0]._attrs;h=true;break;case "node":k=i[0]._nodeAttrs;break;case "edge":k=i[0]._edgeAttrs;break;default:c=new DotNode(f,this,g,i[0]),k=c._attrs,m=c._drawAttrs,i[0]._nodes.push(c)}else if(e=c.match(this._edgeMatchRe))f=e[1],j=e[8],c=new DotEdge(f,this,g,i[0],e[2],e[5]),k=c._attrs,m=c._drawAttrs,i[0]._edges.push(c);if(e){do{if(0==j.length)break;if(e=j.match(this._attrMatchRe)){if(j=
j.substr(e[0].length),c=e[1],l=this._unescape(e[2]),/^_.*draw_$/.test(c)?m[c]=l:k[c]=l,h&&1==i.length)switch(c){case "bb":c=l.split(/,/);this._width=Number(c[2]);this._height=Number(c[3]);break;case "bgcolor":this._bgcolor=g.parseColor(l);break;case "dpi":this._dpi=l;break;case "size":(c=l.match(/^(\d+|\d*(?:\.\d+)),\s*(\d+|\d*(?:\.\d+))(!?)$/))?(this._maxWidth=72*Number(c[1]),this._maxHeight=72*Number(c[2]),this._bbEnlarge="!"==c[3]):debug("can't parse size");break;case "xdotversion":0>this._versionCompare(this.maxXdotVersion,
k.xdotversion)&&debug("unsupported xdotversion "+k.xdotversion+"; this script currently supports up to xdotversion "+this.maxXdotVersion)}}else debug("can't read attributes for entity "+f+" from "+j)}while(e)}}}this._isDirty=true;this._draw();this._graphLoaded=true},_draw:function(a){typeof a==="undefined"&&(a=false);var b=this._scale*this._dpi/72,c=Math.round((this._width+2*this._padding)*this._cacheScale),d=Math.round((this._height+2*this._padding)*this._cacheScale),e=Math.round(b*this._width+2*
this._padding),b=Math.round(b*this._height+2*this._padding);this._canvas.width=e;this._canvas.height=b;if(this._screenWidth==="auto")this._container.style.width=e+"px";if(this._screenHeight==="auto")this._container.style.height=b+"px";if(this._isDirty){if(!a)this._cacheCanvas.width=c,this._cacheCanvas.height=d;this._ctxDraw.save();this._ctxDraw.lineCap="round";this._ctxDraw.fillStyle=this._bgcolor.canvasColor;this._ctxScreen.fillStyle=this._bgcolor.canvasColor;this._ctxDraw.fillRect(0,0,c,d);this._ctxDraw.translate(this._padding,
this._padding);this._graphs[0]._draw(this._ctxDraw,this._cacheScale,a);this._ctxDraw.restore();this._isDirty=false}this._ctxScreen.fillRect(0,0,this._canvas.width,this._canvas.height);this._ctxScreen.drawImage(this._cacheCanvas,0,0,c,d,0,0,e,b)},_drawPath:function(a,b,c,d){c&&(a.beginPath(),b.makePath(a),a.fill());if(a.fillStyle!=a.strokeStyle||!c){switch(d){case "dashed":a.beginPath();b.makeDashedPath(a,this._dashLength);break;case "dotted":var e=a.lineWidth;a.lineWidth*=2;a.beginPath();b.makeDottedPath(a,
this._dotSpacing);break;default:c||(a.beginPath(),b.makePath(a))}a.stroke();if(e)a.lineWidth=e}},_unescape:function(a){var b=a.match(/^"(.*)"$/);return b?b[1].replace(/\\"/g,'"'):a},_parseHexColor:function(a){if(a=a.match(/^#([0-9a-f]{2})\s*([0-9a-f]{2})\s*([0-9a-f]{2})\s*([0-9a-f]{2})?$/i)){var b,c="#"+a[1]+a[2]+a[3],d=1;a[4]?(d=parseInt(a[4],16)/255,b="rgba("+parseInt(a[1],16)+","+parseInt(a[2],16)+","+parseInt(a[3],16)+","+d+")"):b=c}return{canvasColor:b,textColor:c,opacity:d}},_hsvToRgbColor:function(a,
b,c){var d,e,g,h,f,j;a*=360;d=Math.floor(a/60)%6;e=a/60-d;a=c*(1-b);g=c*(1-e*b);b=c*(1-(1-e)*b);switch(d){case 0:h=c;f=b;j=a;break;case 1:h=g;f=c;j=a;break;case 2:h=a;f=c;j=b;break;case 3:h=a;f=g;j=c;break;case 4:h=b;f=a;j=c;break;case 5:h=c,f=a,j=g}return"rgb("+Math.round(255*h)+","+Math.round(255*f)+","+Math.round(255*j)+")"},_versionCompare:function(a,b){for(var a=a.split("."),b=b.split("."),c,d;a.length||b.length;){c=a.length?a.shift():0;d=b.length?b.shift():0;if(c<d)return-1;if(c>d)return 1}return 0},
_handleMouseWheel:function(a){if(this._allowMouseScrolling){if(!a)a=window.event;var b=0;a.wheelDelta?(b=a.wheelDelta/120,window.opera&&(b=-b)):a.detail&&(b=-a.detail/3);b&&this.setScale(this.getScale()+b/10);return DotUtils.preventDefault(a)}},_isValidCssSize:function(a){return typeof a==="string"&&this._cssSize.test(a)},_idMatch:'([a-zA-Z\u0080-\uffff_][0-9a-zA-Z\u0080-\uffff_]*|-?(?:\\.\\d+|\\d+(?:\\.\\d*)?)|"(?:\\\\"|[^"])*"|<(?:<[^>]*>|[^<>]+?)+>)'});
...
Dot.prototype._edgeMatchRe=RegExp("^("+Dot.prototype._nodeIdMatch+"\\s*-[->]\\s*"+Dot.prototype._nodeIdMatch+")\\s+\\[(.+)\\];$");Dot.prototype._attrMatchRe=RegExp("^"+Dot.prototype._idMatch+"="+Dot.prototype._idMatch+"(?:[,\\s]+|$)");Dot.prototype._cssSize=/^\d+(\.\d+)?(px|%)$/;Dot.prototype._cacheScale=3;var DotTokenizer=DotUtils.createClass({initialize:function(a){this._str=a},takeChars:function(a){a||(a=1);for(var b=[];a--;){var c=this._str.match(/^(\S+)\s*/);c?(this._str=this._str.substr(c[0].length),b.push(c[1])):b.push(false)}return 1==b.length?b[0]:b},takeNumber:function(a){a||(a=1);if(1==a)return Number(this.takeChars());else{for(var b=this.takeChars(a);a--;)b[a]=Number(b[a]);return b}},takeString:function(){var a=Number(this.takeChars()),b=0,c;if("-"!=this._str.charAt(0))return false;for(;0<
a;)++b,c=this._str.charCodeAt(b),128>c?--a:a-=2048>c?2:3;a=this._str.substr(1,b);this._str=this._str.substr(1+b).replace(/^\s+/,"");return a}});var DotEntity=DotUtils.createClass({initialize:function(a,b,c,d,e,g){this._defaultAttrHashName=a;this._name=b;this._dot=c;this._rootGraph=d;this._parentGraph=e;this._immediateGraph=g;this._attrs={};this._drawAttrs={}},_initBB:function(){var a=this.getAttr("pos").match(/([0-9.]+),([0-9.]+)/),b=Math.round(a[1]),a=Math.round(this._dot._height-a[2]);this._bbRect=new Rect(b,a,b,a)},getAttr:function(a,b){typeof escStrinng==="undefined"&&(b=false);var c=this._attrs[a];if(typeof c==="undefined")for(var d=
this._parentGraph;typeof d!=="undefined";)if(c=d[this._defaultAttrHashName][a],typeof c==="undefined")d=d._parentGraph;else break;c&&b&&(c=c.replace(this._escStringMatchRe,function(a,b){switch(b){case "N":case "E":return this._name;case "T":return this._tailNode;case "H":return this._headNode;case "G":return this._immediateGraph._name;case "L":return this.getAttr("label",true)}return a}.bind(this)));return c},_draw:function(a,b,c){var d,e,g,h=12;a.lineWidth=b;c||this._initBB();for(daKey in this._drawAttrs){var f=
new DotTokenizer(this._drawAttrs[daKey]);if(d=f.takeChars()){var j="solid";for(a.save();d;){switch(d){case "E":case "e":var l="E"==d,k=f.takeNumber()*b;d=(this._dot._height-f.takeNumber())*b;e=f.takeNumber()*b;var m=f.takeNumber()*b,k=new Ellipse(k,d,e,m);break;case "P":case "p":case "L":var l="P"==d,m="L"!=d,i=f.takeNumber();e=f.takeNumber(2*i);k=new Path;for(d=2;d<2*i;d+=2)k.addBezier([new Point(e[d-2]*b,(this._dot._height-e[d-1])*b),new Point(e[d]*b,(this._dot._height-e[d+1])*b)]);m&&k.addBezier([new Point(e[2*
i-2]*b,(this._dot._height-e[2*i-1])*b),new Point(e[0]*b,(this._dot._height-e[1])*b)]);break;case "B":case "b":l="b"==d;i=f.takeNumber();e=f.takeNumber(2*i);k=new Path;for(d=2;d<2*i;d+=6)k.addBezier([new Point(e[d-2]*b,(this._dot._height-e[d-1])*b),new Point(e[d]*b,(this._dot._height-e[d+1])*b),new Point(e[d+2]*b,(this._dot._height-e[d+3])*b),new Point(e[d+4]*b,(this._dot._height-e[d+5])*b)]);break;case "I":d=f.takeNumber()*b;e=(this._dot._height-f.takeNumber())*b;var m=f.takeNumber()*b,i=f.takeNumber()*
b,o=f.takeString();this._dot._images[o]||(this._dot._images[o]=new DotImage(this._dot,o));this._dot._images[o].draw(a,d,e-i,m,i);break;case "T":d=Math.round(b*f.takeNumber());e=Math.round(this._dot._height-f.takeNumber())*b;m=f.takeNumber();Math.round(b*f.takeNumber());i=f.takeString();if(!c&&!/^\s*$/.test(i)){i=DotUtils.escapeHtml(i);do if(matches=i.match(/ ( +)/)){var n=" ";matches[1].length.times(function(){n+="&nbsp;"});i=i.replace(/  +/,n)}while(matches);a.font="normal normal normal "+h+"px "+
fontFamily;a.textAlign=-1==m?"left":1==m?"right":"center";a.fillStyle=g.textColor;a.fillText(i,d,e)}break;case "C":case "c":d="C"==d;e=this._parseColor(f.takeString());d?a.fillStyle=e.canvasColor:(g=e,a.strokeStyle=e.canvasColor);break;case "F":h=f.takeNumber()*b;fontFamily=f.takeString();switch(fontFamily){case "Times-Roman":fontFamily="Times New Roman";break;case "Courier":fontFamily="Courier New";break;case "Helvetica":fontFamily="Arial"}break;case "S":d=f.takeString();switch(d){case "solid":case "filled":break;
case "dashed":case "dotted":j=d;break;case "bold":a.lineWidth=2*b;break;default:(matches=d.match(/^setlinewidth\((.*)\)$/))?a.lineWidth=Number(matches[1])*b:debug("unknown style "+d)}break;default:debug("unknown token "+d);return}k&&(this._dot._drawPath(a,k,l,j),c||this._bbRect.expandToInclude(k.getBB()),k=void 0);d=f.takeChars()}a.restore()}}},_parseColor:function(a){var b={opacity:1};if(/^#(?:[0-9a-f]{2}\s*){3,4}$/i.test(a))return this._dot._parseHexColor(a);var c=a.match(/^(\d+(?:\.\d+)?)[\s,]+(\d+(?:\.\d+)?)[\s,]+(\d+(?:\.\d+)?)$/);
if(c)return b.canvasColor=b.textColor=this._dot._hsvToRgbColor(c[1],c[2],c[3]),b;var d=this.getAttr("colorscheme")||"X11",e=a;if(c=a.match(/^\/(.*)\/(.*)$/))c[1]&&(d=c[1]),e=c[2];else if(c=a.match(/^\/(.*)$/))d="X11",e=c[1];e=e.toLowerCase();c=d.toLowerCase();if(c=Dot.prototype.colors[c]){var g=c[e];if(g)return this._dot._parseHexColor("#"+g)}if(g=Dot.prototype.colors.fallback[e])return this._dot._parseHexColor("#"+g);c||debug("unknown color scheme "+d);debug("unknown color "+a+"; color scheme is "+
d);b.canvasColor=b.textColor="#000000";return b}});var DotNode=DotUtils.createClass(DotEntity,{initialize:function($super,b,c,d,e){$super("_nodeAttrs",b,c,d,e,e)}});DotNode.prototype.escStringMatchRe=/\\([NGL])/g;var DotEdge=DotUtils.createClass(DotEntity,{initialize:function($super,b,c,d,e,g,h){$super("_edgeAttrs",b,c,d,e,e);this._tailNode=g;this._headNode=h}});DotEdge.prototype._escStringMatchRe=/\\([EGTHL])/g;var DotGraph=DotUtils.createClass(DotEntity,{initialize:function($super,b,c,d,e){$super("_attrs",b,c,d,e,this);this._nodeAttrs={};this._edgeAttrs={};this._nodes=[];this._edges=[];this._subgraphs=[]},_initBB:function(){var a=this.getAttr("bb").split(",");this._bbRect=new Rect(a[0],this._dot._height-a[1],a[2],this._dot._height-a[3])},_draw:function($super,b,c,d){$super(b,c,d);for(var e=[this._subgraphs,this._nodes,this._edges],g=0,h=e.length;g<h;++g)for(var f=e[g],j=0,l=f.length;j<l;++j)f[j]._draw(b,
